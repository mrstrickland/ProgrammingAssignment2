...
makeCacheMatrix <- function(x = matrix()) {
	...
	set<-function(y){ ##Creates a function that sets the value of x and assigns inv as NULL again. Will be used to reset values when x is changed after the inverse of the previous x has been calculated.
	x<<-y
	inv<-NULL
	}
	get <- function() x ##Creates an empty function that reurns the value of x.      
			setinv <- function(a) inv <<- a ##Creates a function that sets inv to be a value outside of the function environment.
...
cacheSolve <- function(x, ...) {
    ...
    if(!is.null(inv)) { ##Tests whether inv is not the value NULL - if this is the case if retrieves the value from the cache as it has already been calculated.
			message("retrieving cache data")
            return(inv) ##Returns the value of the inverse from the Cache
            }
    data <- x$get() ##If the inverse is not already in the cache, it uses the x$get() function to get the value of the matrix x, originally entered into makeCacheMatrix, assigning it to variable data.
